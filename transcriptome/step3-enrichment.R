...
table(nrDEG$change)
head(nrDEG)
sub_nrDEG = nrDEG[nrDEG$change %in% c("UP","DOWN"),]
sub_nrDEG$ENSEMBL=rownames(sub_nrDEG)
library(ggplot2)
library(clusterProfiler)
library(org.Hs.eg.db)
library(stringr)
df <- bitr(unique(sub_nrDEG$ENSEMBL), fromType = "ENSEMBL",
...
head(nrDEG)
table(nrDEG$change)
write.csv(nrDEG,file = "anno_nrDEG_macular.csv")
save(nrDEG,file = 'anno_nrDEG_macular.Rdata')
...
# 包装的函数
{
  run_kegg <- function(gene_up,gene_down,geneList=F,pro='test'){
    ...
    head(kk.up)[,1:6]
    kk=kk.up
    pdf("KK_UP_barplot.pdf",useDingbats = F,width = 8.5,height = 8.5)
    dotplot(kk)
    dev.off()
    ...
    if(geneList){
      ...
      kk=DOSE::setReadable(kk_gse, OrgDb='org.Hs.eg.db',keyType=keyType,)
      tmp=kk@result
...
run_kegg(gene_up,gene_down,pro='kegg_macular_')
...
barplot(go, split="ONTOLOGY",font.size =10)+ 
  ...
  ggsave("go_UP_ALL_barplot.pdf",useDingbats = F,width = 11,height = 8.5) 
go=DOSE::setReadable(go, OrgDb='org.Hs.eg.db',keyType='ENTREZID')
write.csv(go@result,'macular_gene_up_GO_all.csv')
...
barplot(go, split="ONTOLOGY",font.size =10)+ 
  facet_grid(ONTOLOGY~., scale="free") + 
  scale_x_discrete(labels=function(x) str_wrap(x, width=50))+
  ggsave("go_down_ALL_barplot.pdf",useDingbats = F,width = 11,height = 8.5)
go=DOSE::setReadable(go, OrgDb='org.Hs.eg.db',keyType='ENTREZID')
write.csv(go@result,'macular_gene_down_GO_all.csv')

go <- enrichGO(gene_diff, OrgDb = "org.Hs.eg.db", ont="all") 
barplot(go, split="ONTOLOGY",font.size =10)+ 
  facet_grid(ONTOLOGY~., scale="free") + 
  scale_x_discrete(labels=function(x) str_wrap(x, width=50))+
  ggsave("go_diff_ALL_barplot.pdf",useDingbats = F,width = 11,height = 8.5)
go=DOSE::setReadable(go, OrgDb='org.Hs.eg.db',keyType='ENTREZID')
write.csv(go@result,'macular_gene_diff_GO_all.csv')
